    ...
    if(blocco == 'X' || blocco == '#' || blocco == 'S' || blocco == '?' || blocco == 'Q' || 
       blocco == '<' || blocco == '>' || blocco == '[' || blocco == ']' || blocco == 'F')
        ...
        return false;
                            ...
                            //nemici
//-------------------------------------------------------------------------//
enum class TipoNemico{
    goomba,
    koopa
};
enum class StatoNemico{
    vivo,
    schiacciato,
    guscio_in_movimento,
    morto
...
struct Nemico{
    sf::RectangleShape nemico;
    TipoNemico tipo_nemico;
    StatoNemico stato_Nemico = StatoNemico::vivo;

    sf::Vector2f pos_nemico;
    sf::Vector2f velocita_nemico = {0.0f, 0.0f};
    float velocita_camminata = 20.0f;
    float velocita_guscio = 400.0f;

    bool guarda_a_sinistra = true;
    bool nemico_sul_terreno = false;

    float timer_stato = 0.0f; //timer per far sparire il nemico dopo che è stato schiacciato
    float timer_animazione_movimento = 0.0f;
    int frame_corrente = 0;

    sf::Sprite spr_nemico;

    Nemico(TipoNemico tipo, sf::Vector2f pos_iniziale_nemico, const sf::Texture& tex);
    sf::Vector2f getSize();
    sf::FloatRect getBounds();
 
    void collisioniX(const Mondo& mondo);
    void collisioniY(const Mondo& mondo);
    void controllaBordoPiattaforma(const Mondo& mondo);
 
    void aggiuntaFisica(float dt, const Mondo& mondo);
 
    void animazioniGoomba(float dt);
    void animazioniKoopa(float dt);
...
Nemico::Nemico(TipoNemico tipo, sf::Vector2f pos_iniziale_nemico, const sf::Texture& tex) : tipo_nemico(tipo), pos_nemico(pos_iniziale_nemico), spr_nemico(tex){
    spr_nemico.setTexture(tex); 

    if (tipo == TipoNemico::goomba) {
        spr_nemico.setTextureRect(sf::IntRect({0, 16}, {16, 16}));
    } 
    else if (tipo == TipoNemico::koopa) {
        spr_nemico.setTextureRect(sf::IntRect({0, 112}, {16, 24}));
    }

    spr_nemico.setScale({2.0f, 2.0f});
    spr_nemico.setPosition(pos_nemico);
}

sf::Vector2f Nemico::getSize(){
    if(tipo_nemico == TipoNemico::koopa && (stato_Nemico == StatoNemico::schiacciato || stato_Nemico == StatoNemico::guscio_in_movimento))
        return {32.0f, 32.0f};
    if(stato_Nemico == StatoNemico::schiacciato)
        return {32.0f, 16.0f};
    if(tipo_nemico == TipoNemico::goomba)
        return {32.0f, 32.0f};
    if(tipo_nemico == TipoNemico::koopa)
        return {32.0f, 48.0f};
    else
        return {32.0f, 32.0f};
}
...
sf::FloatRect Nemico::getBounds(){
    return sf::FloatRect(pos_nemico, getSize());
}

void Nemico::collisioniX(const Mondo& mondo){
    sf::FloatRect boundNemico = getBounds();

    //ottengo le righe e colonne rispettive della posizione del nemico
    int minC = static_cast<int>(boundNemico.position.x / blocco_size);
    int maxC = static_cast<int>((boundNemico.position.x + boundNemico.size.x) / blocco_size);
    int minR = static_cast<int>((boundNemico.position.y) / blocco_size);
    int maxR = static_cast<int>((boundNemico.position.y + boundNemico.size.y) / blocco_size);

    for(int r = minR; r <= maxR; r++){
        for(int c = minC; c <= maxC; c++){
            //controllo sia all'interno della mappa
            if(r >= 0 && r < static_cast<int>(mondo.mappa.size()) && c >= 0 && c < static_cast<int>(mondo.mappa[r].size()))
            {
                if(BloccoSolido(mondo.mappa[r][c])){
                    ...
                    if(auto inters = boundNemico.findIntersection(hitboxBlocco)){ //quanti px il nemico è dento al blocco
                        if(velocita_nemico.x > 0){
                            pos_nemico.x -= inters->size.x;
                            guarda_a_sinistra = true;
                        }
                        else if(velocita_nemico.x < 0){
                            pos_nemico.x += inters->size.x;
                            guarda_a_sinistra = false;
                        }
                    }
                    
                    boundNemico = getBounds();
                }
            }
        }
    }
}   
void Nemico::collisioniY(const Mondo& mondo){
    sf::FloatRect boundNemico = getBounds();
    nemico_sul_terreno = false;
        ...
        for(int c = minC; c <= maxC; c++){
            ...
            if(r >= 0 && r < static_cast<int>(mondo.mappa.size()) && c >= 0 && c < static_cast<int>(mondo.mappa[r].size()))
            {
                if(BloccoSolido(mondo.mappa[r][c])){
                    ...
                    sf::FloatRect hitboxBlocco({c * blocco_size, r * blocco_size}, {blocco_size, blocco_size});
                    if(auto inters = boundNemico.findIntersection(hitboxBlocco)){ //quanti px il nemico è dento al blocco
                        if(velocita_nemico.y > 0){ //caduta su un blocco
                            pos_nemico.y -= inters->size.y;
                            //pos_nemico.y = (r * blocco_size) - getSize().y;
                            velocita_nemico.y = 0.0f;
                            nemico_sul_terreno = true;
                        }
                        else if(velocita_nemico.y < 0){ //serve se aggiungo la meccanica che se colpisco un blocco dal basso e sopra c'è un nemico esso riceve una spinta negativa
                            pos_nemico.y += inters->size.y;
                            velocita_nemico.y = 0.0f;
                        }
                    }
...
void Nemico::controllaBordoPiattaforma(const Mondo& mondo){
    if(tipo_nemico != TipoNemico::koopa)
        return;

    float controlla_x =  guarda_a_sinistra ? (pos_nemico.x -2.0f) : (pos_nemico.x + getSize().x + 2.0f);
    float controlla_y = pos_nemico.y + getSize().y +2.0f;

    int colonna = static_cast<int>(controlla_x / blocco_size);
    int riga = static_cast<int>(controlla_y / blocco_size);

    if (riga < 0 || riga >= static_cast<int>(mondo.mappa.size()) ||
        colonna < 0 || colonna >= static_cast<int>(mondo.mappa[riga].size()) ||
        !BloccoSolido(mondo.mappa[riga][colonna])) 
    {
        guarda_a_sinistra = !guarda_a_sinistra;
    }
}

void Nemico::aggiuntaFisica(float dt, const Mondo& mondo){
    if(stato_Nemico == StatoNemico::schiacciato || stato_Nemico == StatoNemico::morto)
        return;

    
    // 1. Gestione della gravità e limite di caduta (Terminal Velocity)
    if (!nemico_sul_terreno) {
        velocita_nemico.y += gravita * dt;
        
        const float max_velocita_caduta = 600.0f;
        if (velocita_nemico.y > max_velocita_caduta) {
            velocita_nemico.y = max_velocita_caduta;
        }
    } else {
        velocita_nemico.y = 0.0f; // Azzera la gravità verticale se è poggiato sul terreno
    }

    // Applica il movimento verticale e risolvi le collisioni Y
    pos_nemico.y += velocita_nemico.y * dt;
    collisioniY(mondo);

    // 2. Calcola e applica la velocità orizzontale in base alla direzione
    float vel_x = (tipo_nemico == TipoNemico::koopa && stato_Nemico == StatoNemico::guscio_in_movimento) ? velocita_guscio : velocita_camminata;

    if(tipo_nemico == TipoNemico::koopa && stato_Nemico != StatoNemico::guscio_in_movimento)
        controllaBordoPiattaforma(mondo); //solo se è ancora vivo koopa non deve cadere altrimenti si

    velocita_nemico.x = guarda_a_sinistra ? -vel_x : vel_x;
    pos_nemico.x += velocita_nemico.x * dt;
    collisioniX(mondo);

    // 3. Aggiorna la posizione finale dello sprite
    spr_nemico.setPosition(pos_nemico);
}

void Nemico::animazioniGoomba(float dt){
    const int frame_movimento_x[] = {0, 18};
    const int frame_y = 16;

    if(stato_Nemico == StatoNemico::schiacciato){
        spr_nemico.setTextureRect(sf::IntRect({36, 24}, {16, 8}));
        spr_nemico.setScale({2.0f, 2.0f});
        spr_nemico.setPosition({pos_nemico.x, pos_nemico.y + 16.0f});//per evitare che rimanga sospeso in aria
        return;
    }

    timer_animazione_movimento += dt;
    if(timer_animazione_movimento >= 0.1f){
        timer_animazione_movimento = 0.0f;
        frame_corrente = (frame_corrente + 1) % 2;
    }

    int posX = frame_movimento_x[frame_corrente];
 
    if(!guarda_a_sinistra){
        spr_nemico.setTextureRect(sf::IntRect({posX, frame_y}, {16, 16}));
        spr_nemico.setScale({2.0f, 2.0f});
        spr_nemico.setPosition(pos_nemico);
    }
    else{
        spr_nemico.setTextureRect(sf::IntRect({posX, frame_y}, {16, 16}));
        spr_nemico.setScale({-2.0f, 2.0f});
        spr_nemico.setPosition({pos_nemico.x + 32.0f, pos_nemico.y});
    }
}

void Nemico::animazioniKoopa(float dt){
    const int frame_movimento_x[] = {0, 18};
    const int frame_y = 112;

    if(stato_Nemico == StatoNemico::schiacciato){
        spr_nemico.setTextureRect(sf::IntRect({72, 120}, {16, 16}));
        spr_nemico.setScale({2.0f, 2.0f});
        spr_nemico.setPosition({pos_nemico.x, pos_nemico.y}); //per evitare che rimanga sospeso in aria
        return;
    }
    else if(stato_Nemico == StatoNemico::guscio_in_movimento){
        spr_nemico.setTextureRect(sf::IntRect({90, 120}, {16, 16}));
        spr_nemico.setScale({2.0f, 2.0f});
        spr_nemico.setPosition({pos_nemico.x, pos_nemico.y}); //per evitare che rimanga sospeso in aria
        return;
    }
    ...
    if(!guarda_a_sinistra){
        spr_nemico.setTextureRect(sf::IntRect({posX, frame_y}, {16, 24}));
        spr_nemico.setScale({-2.0f, 2.0f});
        spr_nemico.setPosition({pos_nemico.x + 32.0f, pos_nemico.y});
    }
    else{
...
void Nemico::aggiuntaAnimazioni(float dt){
    switch(tipo_nemico){
        case TipoNemico::goomba:
            animazioniGoomba(dt);
            break;
        case TipoNemico::koopa:
            animazioniKoopa(dt);
            break;
    }
}

void Nemico::update(float dt, const Mondo& mondo){
    aggiuntaFisica(dt, mondo);
    aggiuntaAnimazioni(dt);

    if(stato_Nemico == StatoNemico::schiacciato && tipo_nemico == TipoNemico::goomba){
        timer_stato += dt;
        if(timer_stato > 1.0f)
            stato_Nemico = StatoNemico::morto;
        return;
    }
    if(stato_Nemico == StatoNemico::schiacciato && tipo_nemico == TipoNemico::koopa){
        timer_stato += dt;
        if(timer_stato > 5.0f)
            stato_Nemico = StatoNemico::vivo;
        return;
    }
    if(pos_nemico.y > window_height)
        stato_Nemico = StatoNemico::morto;
}

void Nemico::draw(sf::RenderWindow& window){
    if(stato_Nemico == StatoNemico::morto)
        return;
    window.draw(spr_nemico);
}

                            //palle di fuoco
//-------------------------------------------------------------------------//
enum class StatoPallaDiFuoco{
    in_volo,
    esplosione
};

struct PallaDiFuoco{
    sf::Vector2f pos_palla;
    sf::Vector2f velocita_palla;
    bool attiva = true;
    StatoPallaDiFuoco StatoFuoco = StatoPallaDiFuoco::in_volo;

    sf::Sprite spr_palla;

    float timer_animazionePalla = 0.0f;
    int frame_corrente_palla = 0;

    float gravita_palla = 1800.0f;
    float forza_rimbalzo_palla = -450.0f;

    PallaDiFuoco(sf::Vector2f partenza, float direzione_x, const sf::Texture& tex);
    sf::FloatRect getBounds();

    void collisioniXFuoco(float dt, const Mondo& mondo);
    void collisioniYFuoco(float dt, const Mondo& mondo);
    void collisioniNemiciPalleDiFuoco(float dt, const Mondo& mondo, std::vector<Nemico>& nemici);
    void aggiuntaFisicaPallaDiFuoco(float dt, const Mondo& mondo);
    void aggiuntaAnimazionePallaDiFuoco(float dt);
    void update(float dt, const Mondo& mondo, std::vector<Nemico>& nemici);
    void draw(sf::RenderWindow& window);
};            

PallaDiFuoco::PallaDiFuoco(sf::Vector2f partenza, float direzione_x, const sf::Texture& tex) : spr_palla(tex){
    pos_palla = partenza;
    velocita_palla = {direzione_x, 0.0f};
    spr_palla.setScale({2.0f, 2.0f});
}

sf::FloatRect PallaDiFuoco::getBounds(){
    //-8.0f poichè pos_palla è impostato nel centro
    return sf::FloatRect({pos_palla.x -8.0f, pos_palla.y - 8.0f}, {16.0f, 16.0f});
}

void PallaDiFuoco::collisioniXFuoco(float dt, const Mondo& mondo){
    pos_palla.x += velocita_palla.x * dt;
    
    sf::FloatRect boundsPalla = getBounds();

    int minC = static_cast<int>(boundsPalla.position.x / blocco_size);
    int maxC = static_cast<int>((boundsPalla.position.x + boundsPalla.size.x) / blocco_size);
    int minR = static_cast<int>(boundsPalla.position.y / blocco_size);
    int maxR = static_cast<int>((boundsPalla.position.y + boundsPalla.size.y) / blocco_size);
        ...
        for (int c = minC; c <= maxC; c++) {
            if (r >= 0 && r < static_cast<int>(mondo.mappa.size()) && c >= 0 && c < static_cast<int>(mondo.mappa[r].size())) {
                if (BloccoSolido(mondo.mappa[r][c])) {
                    ...
                    if (auto inters = boundsPalla.findIntersection(hitboxBlocco)) {
                        // Esplode SOLO se l'impatto è laterale (muro), ignorando il pavimento/soffitto
                        if (inters->size.x < inters->size.y) {
                            StatoFuoco = StatoPallaDiFuoco::esplosione;
                            frame_corrente_palla = 0;
                            timer_animazionePalla = 0.0f;
                            return; 
                        }
                    } 
                }
...
void PallaDiFuoco::collisioniYFuoco(float dt, const Mondo& mondo){
    pos_palla.y += velocita_palla.y * dt;
    ...
    for (int r = minR; r <= maxR; r++) {
        for (int c = minC; c <= maxC; c++) {
            if (r >= 0 && r < static_cast<int>(mondo.mappa.size()) && c >= 0 && c < static_cast<int>(mondo.mappa[r].size())) {
                if (BloccoSolido(mondo.mappa[r][c])) {
                    if(velocita_palla.y > 0){
                        pos_palla.y = r * blocco_size - boundsPalla.size.x / 2.0f;
                        velocita_palla.y = forza_rimbalzo_palla;
                    }
                    return;
                }
            }
...
void PallaDiFuoco::collisioniNemiciPalleDiFuoco(float dt, const Mondo& mondo, std::vector<Nemico>& nemici){
    if(StatoFuoco != StatoPallaDiFuoco::esplosione){
        sf::FloatRect boundsPalla = getBounds();

        for(auto& nemico : nemici){
            if(nemico.stato_Nemico != StatoNemico::morto){
                if(boundsPalla.findIntersection(nemico.getBounds())){
                    nemico.stato_Nemico = StatoNemico::morto;
                    StatoFuoco = StatoPallaDiFuoco::esplosione;
                    frame_corrente_palla = 0;
                    timer_animazionePalla = 0.0f;
                }
...
void PallaDiFuoco::aggiuntaFisicaPallaDiFuoco(float dt, const Mondo& mondo){
    //se sta esplodendo o non è attiva non si muove più
    if(!attiva || StatoFuoco == StatoPallaDiFuoco::esplosione)
        return;

    velocita_palla.y += gravita_palla * dt;
    
    collisioniXFuoco(dt, mondo);

    if(StatoFuoco == StatoPallaDiFuoco::esplosione)
        return;

    collisioniYFuoco(dt , mondo);

    if (pos_palla.y > window_height){ //se cade dalla mappa
        attiva = false;
    }
}

void PallaDiFuoco::aggiuntaAnimazionePallaDiFuoco(float dt){
    if(!attiva)
        return;

    const std::vector<sf::IntRect> frames_volo = {
        sf::IntRect({530, 1057}, {8, 8}),   
        sf::IntRect({541, 1057}, {7, 8}),  
        sf::IntRect({551, 1057}, {8, 8}), 
        sf::IntRect({561, 1057}, {8, 7}),   
        sf::IntRect({571, 1057}, {8, 8}),   
        sf::IntRect({581, 1057}, {7, 8}),  
        sf::IntRect({590, 1057}, {8, 8}),  
        sf::IntRect({600, 1057}, {8, 7})   
    };

    const std::vector<sf::IntRect> frames_esplosione = {
        sf::IntRect({611, 1057}, {8, 8}),
        sf::IntRect({624, 1054}, {12, 14}), 
        sf::IntRect({641, 1053}, {16, 16}) 
    };

    timer_animazionePalla += dt;

    if(StatoFuoco == StatoPallaDiFuoco::in_volo){
        if(timer_animazionePalla >= 0.05f){
            timer_animazionePalla = 0.0f;
            frame_corrente_palla = (frame_corrente_palla + 1) % frames_volo.size();
        }
        auto rect_corrente =frames_volo[frame_corrente_palla];
        spr_palla.setTextureRect(rect_corrente);
        spr_palla.setOrigin({rect_corrente.size.x / 2.0f, rect_corrente.size.y / 2.0f});
        spr_palla.setPosition(pos_palla);
    }

    if(StatoFuoco == StatoPallaDiFuoco::esplosione){
        if(timer_animazionePalla >= 0.05f){
            timer_animazionePalla = 0.0f;
            frame_corrente_palla++;

            if(frame_corrente_palla >= frames_esplosione.size()){
                attiva = false;
                return;
            }
        }

        auto rect_corrente =frames_esplosione[frame_corrente_palla];
        spr_palla.setTextureRect(rect_corrente);
        spr_palla.setOrigin({rect_corrente.size.x / 2.0f, rect_corrente.size.y / 2.0f});
        spr_palla.setPosition(pos_palla);
    }
...
void PallaDiFuoco::update(float dt, const Mondo& mondo, std::vector<Nemico>& nemici){
    if (!attiva) return;
    collisioniNemiciPalleDiFuoco(dt, mondo, nemici);
    aggiuntaFisicaPallaDiFuoco(dt, mondo);
    aggiuntaAnimazionePallaDiFuoco(dt);
}

void PallaDiFuoco::draw(sf::RenderWindow& window) {
    if (attiva) {
        window.draw(spr_palla);
    }
}
...
struct Player{
    ...
    float timer_sparo = 0.0f; //per evitare che mario spammmi palle di fuoco
    ...
    float timer_vulnerabilita = 0.0f;
    float durata_vulnerabilita = 2.0f;
    ...
    void inputMovimento(float dt, std::vector<PallaDiFuoco>& palle_di_fuoco);
    void aggiuntaFisica(float dt, const Mondo& mondo);
    ...
    void aggiuntaAnimazioni(float dt);
    void update(float dt, const Mondo& mondo, std::vector<PallaDiFuoco>& palle_di_fuoco);
    void draw(sf::RenderWindow& window);
};
...
void Player::inputMovimento(float dt, std::vector<PallaDiFuoco>& palle_di_fuoco){
    ...
    //gestione sparo
    timer_sparo += dt;
    if(stato == StatoMario::fiore){
        bool premuto_sparo = sf::Keyboard::isKeyPressed(sf::Keyboard::Key::F) || 
                             sf::Mouse::isButtonPressed(sf::Mouse::Button::Left);
        if(premuto_sparo && timer_sparo >= 0.25 && palle_di_fuoco.size() < 2){//max di palle di fuoco a schermo
            timer_sparo = 0.0f;

            float direzione_x = guarda_a_destra ? 400.0f : -400.0f;
            sf::Vector2f pos_sparo = pos + sf::Vector2f(guarda_a_destra ? 32.0f : -8.0f, 16.0f);

            palle_di_fuoco.push_back(PallaDiFuoco(pos_sparo, direzione_x, tex_mov_aggiuntivi));
        }
    }

    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Num1)) {
        stato = StatoMario::piccolo;
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Num2)) {
        stato = StatoMario::grande;
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Num3)) {
        stato = StatoMario::fiore;
    }
...
void Player::update(float dt, const Mondo& mondo,  std::vector<PallaDiFuoco>& palle_di_fuoco) {
    if(timer_vulnerabilita > 0.0f)
        timer_vulnerabilita -=dt;

    inputMovimento(dt, palle_di_fuoco);
    aggiuntaFisica(dt, mondo);
    aggiuntaAnimazioni(dt);
}

void Player::draw(sf::RenderWindow& window){
    // Se è invulnerabile, lampeggia ogni 0.1 secondi
    if (timer_vulnerabilita > 0.0f && (static_cast<int>(timer_vulnerabilita * 10) % 2 == 0)) {
        return; // Non disegna Mario (risultato: scompare e riappare)
    }
...
struct State {
    ...
    std::vector<PallaDiFuoco> palle_di_fuoco;
    sf::View camera;

    sf::Texture tex_nemici;//singola texture condivisa per tutti i nemici
    ...
    void caricaNemici();
    void controllaCollisioniNemici();
    void collisioniMarioNemici();
    void draw(sf::RenderWindow& window);
};

State::State(){
    //carico la texture dei nemici
    sf::Image img_nemici;
    if (!img_nemici.loadFromFile("risorse/NES - Super Mario Bros. - Enemies & Bosses - Enemies & Bosses.png")) {
        std::cerr << "Errore nel caricamento delle immagini dei nemici" << std::endl;
    }

    // Applica la trasparenza dello sfondo
    sf::Color sfondo_azzurro = img_nemici.getPixel({0, 16});
    img_nemici.createMaskFromColor(sfondo_azzurro);
    sf::Color sfondo_blu = img_nemici.getPixel({9, 112});
    img_nemici.createMaskFromColor(sfondo_blu);

    if (!tex_nemici.loadFromImage(img_nemici)) {
        std::cerr << "Errore nella creazione della texture nemici" << std::endl;
    }
...
void State::update(float dt){
    if(!pausa){
        if(mario.scivola_bandiera)
            ...
            mario.update(dt, mondo, palle_di_fuoco);

        for(auto& palla : palle_di_fuoco){
            palla.update(dt, mondo, nemici);
        }
        // Rimozione delle palle non più attive
        for (auto it = palle_di_fuoco.begin(); it != palle_di_fuoco.end(); ) {
            if (!it->attiva) {
                it = palle_di_fuoco.erase(it); // erase invalida l'iteratore e restituisce il successivo
            } else {
                ++it; // avanza solo se l'elemento NON è stato rimosso
            }
        }

        for(auto& n : nemici){
            if(std::abs(n.pos_nemico.x - camera.getCenter().x) < 900.0f){
                n.update(dt, mondo);
            }
        }
        controllaCollisioniNemici();
        collisioniMarioNemici();
        aggiornaCamera();
    } 
}

void State::caricaNemici(){
    for(size_t r = 0; r < mondo.mappa.size(); r++){
        for(size_t c = 0; c < mondo.mappa[r].size(); c++){
            char blocco = mondo.mappa[r][c];

            if(blocco == 'E'){
                sf::Vector2f posizione = {c * blocco_size, (r * blocco_size) - 32.0f};
                //std::cout << "Goomba creato a X: " << posizione.x << " Y: " << posizione.y << std::endl;
                nemici.push_back(Nemico(TipoNemico::goomba, posizione, tex_nemici));
                mondo.mappa[r][c] = '-'; // rimuove la lettera dalla mappa, non deve più contare come tile
            }
            else if(blocco == 'K'){
                sf::Vector2f posizione = {c * blocco_size, (r * blocco_size) - 48.0f}; //-16 perchè koopa è alto 48
                //std::cout << "Koopa creato a X: " << posizione.x << " Y: " << posizione.y << std::endl;
                nemici.push_back(Nemico(TipoNemico::koopa, posizione, tex_nemici));
                mondo.mappa[r][c] = '-';
            }
        }
    }
...
void State::controllaCollisioniNemici(){
    for(size_t i=0; i<nemici.size(); i++){
        if(nemici[i].stato_Nemico == StatoNemico::schiacciato || nemici[i].stato_Nemico == StatoNemico::morto)    
            continue;

        for(size_t j=i+1; j<nemici.size(); j++){
            if(nemici[j].stato_Nemico == StatoNemico::schiacciato || nemici[j].stato_Nemico == StatoNemico::morto)    
                continue;

            sf::FloatRect boundA = nemici[i].getBounds();
            sf::FloatRect boundB = nemici[j].getBounds();

            if(auto inters = boundA.findIntersection(boundB)){
                
                //se entrambi sono vivi inverti le direzioni
                if(nemici[i].stato_Nemico == StatoNemico::vivo && nemici[j].stato_Nemico == StatoNemico::vivo){
                    float sovrapposizione = inters->size.x;
                    float sostamento = sovrapposizione / 2.0f;

                    if(nemici[i].pos_nemico.x < nemici[j].pos_nemico.x){
                        nemici[i].guarda_a_sinistra = true;
                        nemici[j].guarda_a_sinistra = false;
                        nemici[i].pos_nemico.x -= sostamento;
                        nemici[j].pos_nemico.x += sostamento;
                    }
                    else{
                        nemici[i].guarda_a_sinistra = false;
                        nemici[j].guarda_a_sinistra = true;
                        nemici[i].pos_nemico.x += sostamento;
                        nemici[j].pos_nemico.x -= sostamento;
                        
                    }
                    nemici[i].collisioniX(mondo);
                    nemici[j].collisioniX(mondo);
                    nemici[i].spr_nemico.setPosition(nemici[i].pos_nemico);
                    nemici[j].spr_nemico.setPosition(nemici[j].pos_nemico);
                }
                //se uno dei due è un guscio in movimento uccide i nemici che incontra
                else if(nemici[i].stato_Nemico == StatoNemico::guscio_in_movimento && nemici[j].stato_Nemico == StatoNemico::vivo){
                    nemici[j].stato_Nemico = StatoNemico::morto;
                    continue;
                }
                else if(nemici[i].stato_Nemico == StatoNemico::vivo && nemici[j].stato_Nemico == StatoNemico::guscio_in_movimento){
                    nemici[i].stato_Nemico = StatoNemico::morto;
                    continue;
                }
                //se entrambi i gusci sono in movimento vengono eliminati entrambi
                else if(nemici[i].stato_Nemico == StatoNemico::guscio_in_movimento && nemici[j].stato_Nemico == StatoNemico::guscio_in_movimento){
                    nemici[i].stato_Nemico = StatoNemico::morto;
                    nemici[j].stato_Nemico = StatoNemico::morto;
                    continue;
                }
...
void State::collisioniMarioNemici(){
    for(auto& n : nemici){
        if(n.stato_Nemico == StatoNemico::morto || (n.tipo_nemico == TipoNemico::goomba && n.stato_Nemico == StatoNemico::schiacciato))    
            continue;

        sf::FloatRect boundMario = mario.getBounds();
        sf::FloatRect boundNemico = n.getBounds();

        if(auto inters = boundMario.findIntersection(boundNemico)){
            bool arriva_dall_alto = (mario.velocita.y > 0) && (boundMario.position.y < n.pos_nemico.y);

            if(arriva_dall_alto){
                if(n.tipo_nemico == TipoNemico::goomba){
                    n.stato_Nemico = StatoNemico::schiacciato;
                    n.timer_stato = 0.0f;
                    n.velocita_nemico = {0.0f, 0.0f};
                }
                else if(n.tipo_nemico == TipoNemico::koopa){
                    if(n.stato_Nemico == StatoNemico::vivo){
                        n.stato_Nemico = StatoNemico::schiacciato;
                        n.pos_nemico.y += 16.0f; //per evitare che rimanga rialzato
                        n.timer_stato = 0.0f;
                        n.velocita_nemico = {0.0f, 0.0f};
                    }
                    else if(n.stato_Nemico == StatoNemico::guscio_in_movimento){
                        n.stato_Nemico = StatoNemico::schiacciato;
                        n.timer_stato = 0.0f;
                        n.velocita_nemico = {0.0f, 0.0f};
                    }
                    else if(n.stato_Nemico == StatoNemico::schiacciato){
                        n.stato_Nemico = StatoNemico::morto;
                    }
                }
                mario.velocita.y = forza_salto;
                mario.sul_terreno = false;
            }

            else if(n.tipo_nemico == TipoNemico::koopa && n.stato_Nemico == StatoNemico::schiacciato){
                if(mario.pos.x < n.pos_nemico.x){
                    n.guarda_a_sinistra = false; //guscio va verso destra
                    //per evitare la doppia hit
                    n.pos_nemico.x += inters->size.x; 
                    mario.velocita.x = 0.0f;
                }
                else{
                    n.guarda_a_sinistra = true; //guscio va verso sinistra
                    //per evitare la doppia hit
                    n.pos_nemico.x -= inters->size.x; 
                    mario.velocita.x = 0.0f;
                }
                n.stato_Nemico = StatoNemico::guscio_in_movimento;
            }

            else if(mario.timer_vulnerabilita <= 0.0f){
                if(n.tipo_nemico == TipoNemico::koopa && n.stato_Nemico == StatoNemico::guscio_in_movimento)
                    n.guarda_a_sinistra = !n.guarda_a_sinistra;

                if(mario.stato == StatoMario::fiore){
                    mario.stato =  StatoMario::grande;
                    mario.timer_vulnerabilita = mario.durata_vulnerabilita;
                }
                else if(mario.stato == StatoMario::grande){
                    mario.stato =  StatoMario::piccolo;
                    mario.timer_vulnerabilita = mario.durata_vulnerabilita;
                }
                else if(mario.stato ==  StatoMario::piccolo)
                    mario.pos = mario_pos_iniziale;
                mario.velocita = {0.0f, 0.0f};
            }
        }
    }
}
...
int main(){
    ...
    window.setPosition(sf::Vector2i(
        desktop.size.x+(desktop.size.x - window.getSize().x) / 2,
    ...
    while(window.isOpen()){
        ...
        if (dt > 0.05f) {
            dt = 0.05f; // Impedisce che il tempo avanzi troppo in un solo frame in caso di lag
        }
